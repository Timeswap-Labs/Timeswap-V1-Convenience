    ...
    await fc.assert(
      fc.asyncProperty(
        ...
        //   console.log('ts updated balance',liquidityBalanceNew - data.removeLiquidityParams.liquidityIn);
          ...
          const totalLiquidityBalanceContract = await TimeswapPair__factory.connect(await result.convenience.factoryContract.getPair(result.assetToken.address,result.collateralToken.address),ethers.provider).totalLiquidity(maturity)
